...
class CustomFeatureEngineering(BaseEstimator, TransformerMixin):
    def fit(self, X, y=None):
        self.transplant_categories_ = [
            'Kidney', 'heart', 'heart-kidney', 'heart-lungs', 'lung', 'lung-kidney', 'nothing']
        return self
    ...
    def transform(self, X):
        ...
        X_encoded['Gender'] = X_encoded['Gender'].map(
            {'Female': -1, 'Male': 1})
        ...
        X_encoded['Age_Heart_Interaction'] = X_encoded['Age'] * \
            X_encoded['Heart_Condition_Severity_Index']
        X_encoded['Age_Lung_Interaction'] = X_encoded['Age'] * \
            X_encoded['Lung_Condition_Severity_Index']
        X_encoded['Age_Kidney_Interaction'] = X_encoded['Age'] * \
            X_encoded['Kidney_Condition_Severity_Index']
        X_encoded['Age_Chronic_Interaction'] = X_encoded['Age'] * \
            X_encoded['Chronic_Condition_Severity_Index']

        X_encoded['Gender_Heart_Interaction'] = X_encoded['Gender'] * \
            X_encoded['Heart_Condition_Severity_Index']
        X_encoded['Gender_Kidney_Interaction'] = X_encoded['Gender'] * \
            X_encoded['Kidney_Condition_Severity_Index']
        X_encoded['Gender_Lung_Interaction'] = X_encoded['Gender'] * \
            X_encoded['Lung_Condition_Severity_Index']
...
non_scaled_columns = ['Gender', 'Kidney', 'heart',
...
def drop_columns_if_exist(X):
    ...
    df = pd.DataFrame(X, columns=[str(col)
                      for col in (scale_columns + non_scaled_columns)])
...
def preprocess(df):
    ...
    X_scaled[scale_columns] = scaler.fit_transform(
        X_transformed[scale_columns])
...
def predict_pipeline(data: dict) -> list:
    ...
    for object in data.values():
        ...
        df['Heart Defect at birth'].append(
            object["conditions"]["heartDefectAtBirth"])
        df['Cardiomyopathy'].append(object["conditions"]["cardiomyopathy"])
        df['Severe cystic fibrosis'].append(
            object["conditions"]["severeCysticFibrosis"])
        df['copd(lung_Disease)'].append(object["conditions"]["copd"])
        df['Repeated urinary infections'].append(
            object["conditions"]["repeatedUrinaryInfections"])
        df['Diabities'].append(object["conditions"]["diabities"])
        df['kidney stones'].append(object["conditions"]["kidneyStones"])
        df['Urinary Tract Infection'].append(
            object["conditions"]["urinaryTractInfection"])
